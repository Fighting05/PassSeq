...
class Trainer(object):
    ...
    def train(self):
        ...
        while True:
            ...
            # if self.epoch == 5:
            #     # self.model.eval()
            #     # file = open('D:/PII_generation/sent_summarizer-master/generate.txt', 'w', encoding='utf-8')
            #     # with torch.no_grad():
            #     #     for srcs, targets in self.test_loader:
            #     #         src, src_lens, tgt, extend_src, ext_vsize = srcs
            #     #         # src, src_lens, tgt = srcs
            #     #         extend_src = extend_src.to(self.device)
            #     #         src, src_lens, tgt, targets = src.to(self.device), \
